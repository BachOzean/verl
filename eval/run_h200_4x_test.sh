...
DATASET="/home/ningmiao/ningyuan/verl/data/OpenR1-Math-220k/small"
SPLIT="train"
OUTPUT_DIR="$HOME/verl/eval/results/OpenR1-Math-220k_H200_4x_test"
NUM_SAMPLES=16
SAMPLES_PER_CALL=16
BATCH_SIZE=128
NUM_GPUS=4
...
echo "🚀 4x H200 启动多vLLM Server + 客户端批推理..."
echo "  模型: $MODEL"
echo "  数据集: $DATASET"
...
for i in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    CMD="export CUDA_VISIBLE_DEVICES=$GPU_ID && /opt/anaconda3/envs/xny_verl/bin/python -m vllm.entrypoints.openai.api_server --model '$MODEL' --tensor-parallel-size 1 --dtype $DTYPE --max-model-len $MAX_MODEL_LEN --gpu-memory-utilization $GPU_MEMORY --trust-remote-code --port $PORT"
